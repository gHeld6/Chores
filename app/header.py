from app.models import db, Chore, User
...
def check_time_for_notify(now, notify_hour):
    if (notify_hour == int(now.strftime("%H"))) and now.strftime("%M") == "00":
        return True
    return False
...
def get_days():
    ...
    for chore in chores:
        days[chore.day].append({"chore": chore.chore, "user": User.query.filter_by(id=chore.user_id).first(),
             "complete": chore.completed, "id": chore.id, "notify_time": chore.time_completed_by,
             "recurring": chore.recurring})
    return days


def notify(name_str, chore_str, title):
    tok = User.query.filter_by(name=name_str).first().token
    body = "{}!!! Chore: {} needs to be finished!".format(name_str, chore_str)
    send_data = {"type": "note", "title": title, "body": body}
    resp = requests.post('https://api.pushbullet.com/v2/pushes', data=json.dumps(send_data),
